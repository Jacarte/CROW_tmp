...
docker run --rm -v $PROGRAM_DIR:/inputs -v $(pwd)/crow/crow/settings:/slumps/crow/settings  -v $(pwd)/crow_out:/slumps/crow/out  -v $(pwd)/logs:/slumps/crow/logs  --entrypoint /bin/bash jacarte/slumps:crow ./start_poly_bench.sh %DEFAULT.timeout 2000 %DEFAULT.link-wasi False %DEFAULT.outfolder /slumps/crow/out   %DEFAULT.exploration-timeout 1000   /inputs/$PROGRAM_NAME
